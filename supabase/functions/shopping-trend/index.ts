...
function isPotentialContentTopic(keyword: string) {
  return isBlogCandidate(keyword)
    || /기온|무더위|더위|한파|폭설|태풍|집중호우|호우|침수|습도|냉방|난방|전기료|에너지비|관리비|실내온도|주거|주택|아파트|리모델링|인테리어|셀프시공|DIY|캠핑|차량온도|자동차열|차량커튼|방수|환기|겨울|여름|환절기|미세먼지|가습기|보일러|난방비|전기세|가스비|온풍기|히터|장판|바닥|외풍|문풍지|뽁뽁이|샷시|창호|방음|흡음|건조|온수매트|전기장판/i.test(keyword);
}
...
const NICHE_BASE_SEEDS = [
  ...
  "준불연 단열재", "결로", "벽 곰팡이", "리모델링 단열",
  // 불연/난연 성능·인증 이슈 — 사용자 지정(2026-09-07, "불연재라고 시험 면제했는데... 화재안전
  // 구멍" 기사가 준불연/불연 단열재를 취급하는데도 안 잡혔던 사례로 발견). 준불연 단열재 하나만
  // 시드로는 "불연재" 자체를 다루는 인증·시험 관련 기사가 검색에 안 걸린다.
  "불연재", "샌드위치패널 화재", "건축자재 방염",
  // 자사·경쟁사 블로그 91/439건 실제 제목으로 검증(2026-09-07) — 글라스울/미네랄울은
  // rank-tracker.html의 "불연열반사" 카테고리에 이미 정식 제품군인데 여기 시드엔 빠져있었다.
  "글라스울", "은박단열재",
  // 정책/규제 — 신축·리모델링 규제가 바뀌면 수요에 바로 영향을 주는데 지금은 제로에너지/패시브
  // 정도만 있어서 촘촘하지 않았다(2026-09-07 사용자 지정).
  "그린리모델링", "제로에너지건축물 인증", "건축물 에너지효율등급",
  // 원자재/공급망 — 자사 글에도 "단열재 수급 불안정" 언급이 있었을 만큼 원가에 민감한 이슈인데
  // 지금 시드엔 이 각도가 전혀 없었다.
  "단열재 원자재 가격", "폴리스티렌 가격",
  // 전문용어 말고 실생활에서 쓰는 말도 필요하다는 지적(2026-09-07) — 뽁뽁이/문풍지/웃풍은
  // domainPattern에도 추가, 베란다는 검색 시드로만.
  "베란다 단열",
  // 외벽 단열재/마감재 탈락·붕괴 사고 — "단열재"란 검색어로는 이런 사고 기사가 애초에
  // 검색에 안 걸릴 수 있어서(제목 필터만 고쳐선 안 되고, 검색 자체가 이 기사를 가져와야 함)
  // 시드로 직접 추가한다(2026-09-07, "포천 빌라 외벽 대규모 붕괴" 사례로 발견).
  "외벽 붕괴", "외벽 마감재 탈락",
  // 건축현장 사고·건축물 화재까지 확장(2026-09-07 사용자 지정).
  "건축현장 사고", "건설현장 사고", "건축물 화재", "공장 화재",
];
...
async function collectNicheTrendData() {
  ...
  // 불연/난연/방염/내화/샌드위치패널/복합자재/심재 — 화재안전 인증·시험 관련 기사(2026-09-07
  // 사용자 지정). 기존엔 "준불연"만 있어서 "불연재" 자체를 다루는 기사(예: 불연재 시험 면제
  // 특혜 의혹, 화재안전 부실 인증)가 제목 필터에서 전부 걸러졌었다.
  // 글라스울/미네랄울/은박/페놀폼과 열전도율·열관류율·열저항(스펙만 다루고 "단열" 단어가 아예
  // 없는 기사, 예: 자사 글 "각 재료별 열전도율") — 자사·경쟁사 블로그 실제 제목 530건으로
  // 검증해서 추가(2026-09-07). 그린리모델링/에너지효율등급/폴리스티렌(정책·원자재)과
  // 뽁뽁이/문풍지/웃풍(전문용어 아닌 실생활 용어)도 같이 추가.
  // 드라이비트(외단열마감공법) — 외벽 마감재/단열재 탈락·붕괴 사고 기사에 실제로 자주 쓰이는
  // 특정 용어(2026-09-07, 자사 블로그 "드라이비트의 진실" 글로 이미 다뤘던 용어).
  // 외벽 — 붕괴/탈락 사고 기사의 관련성 점수(relevanceHits)에도 잡히도록 domainPattern에도
  // 추가한다(2026-09-07). 전에 facadeFailurePattern만 있어서 제목 필터는 통과해도, 이슈
  // 점수 계산의 "관련성" 항목에서 이 단어를 못 알아봐 점수가 낮게 나오고 있었다(단독보도라
  // 보도량·언론사 다양성 점수도 낮아서 상위 20위 밖으로 밀림).
  const domainPattern = /단열|결로|곰팡이|방수|냉방|난방|폭염|열대야|장마|제습|습기|에어컨|실외기|창문|창호|샷시|햇빛|열차단|외풍|보일러|리모델링|집수리|에너지효율|에너지 효율|녹색건축|제로에너지|패시브|우레탄|아이소핑크|스티로폼|준불연|PF보드|단열필름|불연|난연|방염|내화|샌드위치패널|복합자재|심재|글라스울|미네랄울|은박|페놀폼|열전도율|열관류율|열저항|그린리모델링|에너지효율등급|폴리스티렌|뽁뽁이|문풍지|웃풍|드라이비트|외벽/i;
  ...
  // 건축물 사고 전반 — 외벽 붕괴/탈락/낙하, 건축·건설·공사 현장 사고, 건축물·공장·주택·아파트·
  // 빌라·상가 화재. 사고 기사는 "단열재"란 말 없이 사고 용어만 쓰는 경우가 많아 domainPattern을
  // 못 통과했다(2026-09-07, "포천 빌라 외벽 대규모 붕괴" 사례로 발견 후 사용자 지정으로 건축
  // 현장사고·건축물 화재까지 확장). "단열" 단어가 없어도 이 조합이면 통과시킨다.
  const constructionAccidentPattern =
    /외벽.{0,10}(붕괴|탈락|낙하)|(붕괴|탈락|낙하).{0,10}외벽|(건축|건설|공사)\s?현장.{0,10}사고|사고.{0,10}(건축|건설|공사)\s?현장|(건축물|건물|공장|주택|아파트|빌라|상가).{0,10}화재|화재.{0,10}(건축물|건물|공장|주택|아파트|빌라|상가)/;
  const isRelevantTitle = (title: string) => {
    if (!title || noisePattern.test(title)) return false;
    if (constructionAccidentPattern.test(title)) return true;
    if (!domainPattern.test(title)) return false;
