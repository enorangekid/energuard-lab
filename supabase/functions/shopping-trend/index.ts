...
async function collectNicheTrendData() {
  ...
  const domainPattern = /단열|결로|곰팡이|방수|냉방|난방|폭염|열대야|장마|제습|습기|에어컨|실외기|창문|창호|샷시|햇빛|열차단|외풍|보일러|리모델링|집수리|에너지효율|에너지 효율|녹색건축|제로에너지|패시브|우레탄|아이소핑크|스티로폼|준불연|PF보드|단열필름|불연|난연|방염|내화|샌드위치패널|복합자재|심재|글라스울|미네랄울|은박|페놀폼|열전도율|열관류율|열저항|그린리모델링|에너지효율등급|폴리스티렌|뽁뽁이|문풍지|웃풍|드라이비트/i;
  ...
  // 외벽 붕괴·탈락·낙하 사고 — 실제론 외벽 단열재/마감재가 떨어지는 사고인데, 사고 기사는
  // "단열재"란 말 없이 "붕괴"/"탈락"/"낙하"만 쓰는 경우가 많아 domainPattern을 통과 못 했다
  // (2026-09-07, "포천 빌라 외벽 대규모 붕괴" 사례로 발견). "단열" 단어가 없어도 이 조합이면
  // 통과시킨다.
  const facadeFailurePattern = /외벽.{0,10}(붕괴|탈락|낙하)|(붕괴|탈락|낙하).{0,10}외벽/;
  const isRelevantTitle = (title: string) => {
    if (!title || noisePattern.test(title)) return false;
    if (facadeFailurePattern.test(title)) return true;
