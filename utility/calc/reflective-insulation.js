...
function calculate() {
  ...
  // 평당 소요량: 1평(3.30579m²) ÷ 폭1M = 3.31m
  const perPyeong     = r(3.30579 / ROLL_WIDTH_M, 1);

  document.getElementById('resMainSub').textContent  = `${netM2display} m²에 필요한 열반사단열재 길이`;
  ...
  const deductNote = hasDeduct && deductDetails.length
    ...
    : '';
  noteEl.innerHTML = `
    ※ ${deductNote}최종 수량 <strong>${finalLength}m</strong>은 할증(${lossRate}%)을 포함한 발주 권장 길이입니다. (제품 폭 1M 기준)
    <br/>※ 실제 현장 조건에 따라 수량이 달라질 수 있습니다.
  `;

  addHistory(finalLength, netM2display, lossRate);
}
